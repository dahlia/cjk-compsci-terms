...
Deno.test("romanize handles Traditional Chinese with Pinyin", async () => {
  const result = await romanize("電腦", "zh-TW");
  assertEquals(result.langTag, "zh-TW-Latn-pny");
  assertEquals(result.text.length > 0, true);
  // Should be ASCII with possible diacritics (pinyin)
  assertEquals(/^[A-Za-zāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜ]+$/.test(result.text), true);
});
